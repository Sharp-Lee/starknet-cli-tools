...
import { checkImages } from "./utils/insetImages.mjs";
...
async function performTasks(starkAccount, ethAccount) {
    ...
    while (!accountData.naming || !accountData.mint_square || !accountData.my_swap || !accountData.jedi_swap) {
        ...
        for (const dapp of shuffledDappAddresses) {
            ...
            // 如果是本次交互的, 随机等待10-30秒
            if (interacted) {
                await new Promise((resolve) => setTimeout(resolve, randomInt(10 * 1000, 20 * 1000)));
            }
        }
        accountData = await db.get("SELECT * FROM accounts WHERE starknet_address = ? AND eth_address = ?", [starkAccount.account.address, ethAccount.address]);
    }
    ...
    while (true) {
        ...
        const delay = randomInt(10 * 1000, 20 * 1000);
...
async function main() {
    ...
    try {
        ...
        for (let i = 0; i < starkAccounts.length; i++) {
            ...
            // 模拟用户使用时间的随机性, 每间隔10-30秒有一个新用户进入交互
            const delay = randomInt(1 * 1000, 3 * 1000) + lastDelay;
            lastDelay = delay;
